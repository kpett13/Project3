...
P1 = 0.5*10**4            # Low side pressure
pr = 18                   # Pressure ratio
...
for mdot in range(1,10):
    ...
    mdot_WF = (mdot/500)   # Define actual mdot of working fluid (kg/s)
    
    "State 1 - Outlet Throttle / Inlet Evaporator"
    X1 = 0                 # Fluid is assumed to be a saturated liquid
    ...
    h2 = (q_evaporator/mdot_WF)+h1   # Heat from the source is q = mdot*(delta:h)          
...
#    WF_4_perf =  ct.Hfc134a()
    ...
    _Vel_Liq.append(mdot_WF/(Ac_Liqline*WF_1.density))  
    _Vel_Gas.append(mdot_WF/(Ac_Gasline*WF_3.density))
    _e_Rad.append(e_Rad)
    _T1.append(T1)
    _T2.append(T2)
    _T3.append(T3)
